#=
Robustness analysis for static seeding options.

One static option is applied for the whole seeding window (contrast with
`robustness_analysis_pathways.jl`, where the option changes over time). For each parameter set
(N_seed budget, min_iv_locations, RCP, DHW) each option is CVaR-aggregated against a
no-intervention counterfactual on three metrics: years >20% coral cover, cumulative absolute
cover, cumulative coral evenness.

Analysis lives in `robustness_data_processing.jl`; plotting in `robustness_visualisation.jl`.
This script only orchestrates: build & run scenarios → aggregate metrics → make plots.
=#
